...
public class Drawer2(private val variations: Array<out DrawerVariant>) :
            ...
            div(
                ...
                div(baseClass = "drawer".component("main")) {
                    ...
                    div(
                        ...
                        if (resizable) {
                            inlineStyle(splitterPosition.data.filter { it > 0 }.map { position ->
    ...
    private fun handleKey(event: Event) {
        ...
        if (keyResizing) {
            val multiplier = if (event.unsafeCast<KeyboardEvent>().shiftKey) 10 else 1
            val rect = panelRect ?: panelElement.getBoundingClientRect()
            val delta = when (key) {
...
/**
 * The primary container of a [drawer][Drawer2] component which is always visible.
 *
...
/**
 * The detail container of a [drawer][Drawer2] component which slides in from the edge of the viewport.
 *
